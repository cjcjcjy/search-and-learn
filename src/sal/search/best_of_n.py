...
def best_of_n(x, config: Config, prm: PRM):
# def best_of_n(x, config: Config, llm: LLM):
    ...
    if len(responses) != len(x["question"]) * config.n:
        raise ValueError(
            f"Generated {len(responses)} responses instead of {len(x['question'] * config.n)}"
        )
