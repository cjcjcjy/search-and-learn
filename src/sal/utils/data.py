...
def get_dataset(config: Config) -> Dataset:
    ...
    if config.dataset_name == "HuggingFaceH4/MATH-500":
        dataset = load_dataset("HuggingFaceH4/MATH-500", split="test")

    if config.dataset_name == "opencompass/AIME2025":
        dataset1 = load_dataset(config.dataset_name, "AIME2025-I", split="test")
        dataset2 = load_dataset(config.dataset_name, "AIME2025-II", split="test")
        dataset = concatenate_datasets([dataset1, dataset2])

    if config.dataset_name == "HuggingFaceH4/aime_2024":
        dataset = load_dataset(config.dataset_name, split = "train")
        
    # aime2023
    if config.dataset_name == "xiaomama2002/AIME-2022-2025":
        dataset = load_dataset(config.dataset_name, split = "train")
        config.dataset_start = 30
        config.dataset_end = 60
