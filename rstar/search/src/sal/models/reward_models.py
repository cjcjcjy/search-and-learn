...
def batched_math_shepherd_inference(
    ...
    for i in range(0, len(inputs), batch_size):
        ...
        logging.info(f"inputs_batch={inputs_batch}")
        ...
        with torch.no_grad():
            ...
            logging.info(f"step_scores={step_scores}")  
...
class RLHFFlow(PRM):
    def load_model_and_tokenizer(
        ...
        #     # "/data/disk2/jjxiao/peiyi9979/math-shepherd-mistral-7b-prm"
    ...
    def _score_batched(
        ...
        # special_tok_id = self.tokenizer("\n\n\n\n\n", return_tensors="pt").input_ids[0, 1]
        # 添加特殊标记
        self.tokenizer.add_tokens(["\n\n\n\n\n"])
        # 获取特殊标记的 token ID
        special_tok_id = self.tokenizer.convert_tokens_to_ids("\n\n\n\n\n")
...
def load_prm(config: Config) -> PRM:
    ...
    if config.prm_path == "openreasoner/Math-psa":
        return RLHFFlow(config)
...
class QwenProcessRM(nn.Module):
    ...
    def __init__(self, search_config: Config, all_args):
        super().__init__()
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/data/disk2/jjxiao/Qwen/Qwen2.5-Math-7B-Instruct", add_eos_token=False, padding_side='left')
        self.tokenizer.pad_token_id = 151655 # "<|image_pad|>"
        self.candidate_tokens = self.tokenizer.encode(f" {self.good_token} {self.bad_token}") # [488, 481]
        self.step_tag_id = self.tokenizer.encode(f" {self.step_tag}")[-1] # 76325
        self.model = AutoModelForCausalLM.from_pretrained("/data/disk2/jjxiao/Qwen/Qwen2.5-Math-1.5B-Instruct", 
                                                          device_map="auto", 
                                                          torch_dtype=torch.bfloat16,
                                                        #   attn_implementation="flash_attention_2",
                                                          ).eval()
    ...
    def score(
        ...
        # TODO: tokenize each batch independently so there is less padding and faster inference
        output_scores = self.batched_math_shepherd_inference_new(
            self.model,
            self.tokenizer,
            inputs_for_prm,
            self.search_config.prm_batch_size,
        ...
        return output_scores
    def batched_math_shepherd_inference_new(
        self,
        model: PreTrainedModel,
        tokenizer: PreTrainedTokenizer,
        inputs: list[str],
        ...
        for i in range(0, len(inputs), batch_size):
            ...
            with torch.no_grad():
                logits = model(**inputs_batch).logits[:, :, self.candidate_tokens]
                scores = logits.softmax(dim=-1)[:, :, 0]
                step_scores_flat = scores[inputs_batch.input_ids == STEP_TAG_ID].tolist()
                # Split scores into sublist based on number of \n in the input
                step_scores = []
